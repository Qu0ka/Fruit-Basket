...
Vous vous trouvez dans le fichier trajbanane.py de notre projet.
...
class Trajbanane(pygame.sprite.Sprite):
    """ Cette classe s'occupe de la gestion du fruit qui lui est associé"""
    def __init__(self, player):
        self.panier = False
        super().__init__()
        self.player = player
        self.image = pygame.image.load('banane_blank.png')
    ...
    def remove(self):
        ...
        self.player.banane.remove(self)

    def move_banane(self, x, y, i):
        """Fonction qui gère le déplacement de la banane"""
        self.i = i
        ...
        if self.panier is False:
            ...
            if self.rect.x > 1080 or self.rect.y > 720 or i == 499:
                ...
                return lancer_banane_possible

    def panier_banane(self, angle):
        """Fonction qui détecte si un panier a été marqué"""
        centre_banane_x = self.rect.x - 100
        centre_banane_y = self.rect.y - 75
        print(centre_banane_x, self.rect.x, centre_banane_y, self.rect.y)
        if 860 < centre_banane_x < 880 and 160 < centre_banane_y < 180 and angle > 50:
            self.panier = True
            self.remove()
            return True
        return False

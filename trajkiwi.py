...
Vous vous trouvez dans le fichier trajkiwi.py de notre projet.
...
class Trajkiwi(pygame.sprite.Sprite):
    """ Cette classe s'occupe de la gestion du fruit qui lui est associé"""
    def __init__(self, player):
        self.panier = False
        super().__init__()
        self.player = player
        self.image = pygame.image.load('kiwi_blank.png')
    ...
    def remove(self):
        ...
        self.player.kiwi.remove(self)

    def move_kiwi(self, x, y, i):
        """Fonction qui gère le déplacement du kiwi"""
        self.i = i
        ...
        if self.panier is False:
            ...
            if self.rect.x > 1080 or self.rect.y > 720 or i == 499:
                ...
                return lancer_kiwi_possible

    def panier_kiwi(self, angle):
        """Fonction qui détecte si un panier a été marqué"""
        centre_kiwi_x = self.rect.x - 100
        centre_kiwi_y = self.rect.y - 75
        print(centre_kiwi_x, self.rect.x, centre_kiwi_y, self.rect.y)
        if 860 < centre_kiwi_x < 880 and 160 < centre_kiwi_y < 180 and angle > 50:
            self.panier = True
            self.remove()
            return True
        return False

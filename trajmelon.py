...
Vous vous trouvez dans le fichier trajmelon.py de notre projet.
...
class Trajmelon(pygame.sprite.Sprite):
    """ Cette classe s'occupe de la gestion du fruit qui lui est associé"""
    def __init__(self, player):
        self.panier = False
        super().__init__()
        self.player = player
        self.image = pygame.image.load('melon_blank.png')
        self.image = pygame.transform.scale(self.image, (115, 100))
        self.rect = self.image.get_rect()
        self.rect.x = player.rect.x + 140
        self.rect.y = player.rect.y + 20
        self.x_init = player.rect.x + 15
        self.y_init = player.rect.y - 380
        self.mass = 6

    def remove(self):
        """Fonction qui supprime le sprite en cours"""
        self.player.melon.remove(self)

    def move_melon(self, x, y, i):
        """Fonction qui gère le déplacement du melon"""
        self.i = i
        ...
        if self.panier is False:
            ...
            if self.rect.x > 1080 or self.rect.y > 720 or i == 499:
                ...
                return lancer_melon_possible

    def panier_melon(self, angle):
        """Fonction qui détecte si un panier a été marqué"""
        centre_melon_x = self.rect.x - 200
        centre_melon_y = self.rect.y - 70

        if 708 < centre_melon_x < 740 and 140 < centre_melon_y < 150 and angle > 50:
            self.panier = True
            self.remove()
            return True
        return False

...
Vous vous trouvez dans le fichier trajfraise.py de notre projet.
...
class Trajfraise(pygame.sprite.Sprite):
    """ Cette classe s'occupe de la gestion du fruit qui lui est associé"""
    def __init__(self, player):
        self.panier = False
        super().__init__()
        self.player = player
        self.image = pygame.image.load('fraise_blank.png')
    ...
    def remove(self):
        ...
        self.player.fraise.remove(self)

    def move_fraise(self, x, y, i):
        """Fonction qui gère le déplacement de la fraise"""
        self.i = i
        ...
        if self.panier is False:
            ...
            if self.rect.x > 1080 or self.rect.y > 720 or i == 499:
                ...
                return lancer_fraise_possible

    def panier_fraise(self, angle):
        """Fonction qui détecte si un panier a été marqué"""
        centre_fraise_x = self.rect.x - 100
        centre_fraise_y = self.rect.y - 75
        if 860 < centre_fraise_x < 880 and 160 < centre_fraise_y < 180 and angle > 50:
            print("Pannier fraise ! Appuyer sur k pour dessiner une nouvelle trajectoire")
            self.panier = True
            self.remove()
            return True
        return False

...
Vous vous trouvez dans le fichier trajananas.py de notre projet.
...
class Trajananas(pygame.sprite.Sprite):
    def __init__(self, player):
        self.panier = False
        super().__init__()
        self.player = player
        self.image = pygame.image.load('ananas_blank.png')
    ...
    def remove(self):
        ...
        self.player.ananas.remove(self)

    def move_ananas(self, x, y, i):
        ...
        if self.panier is False:
            ...
            if self.rect.x > 1080 or self.rect.y > 720 or i == 499:
                ...
                return lancer_ananas_possible

    def panier_ananas(self, angle):

        centre_ananas_x = self.rect.x - 100
        centre_ananas_y = self.rect.y - 75
        if 845 < centre_ananas_x < 860 and 95 < centre_ananas_y < 120 and angle > 50:
            self.panier = True
            self.remove()
            return True
        return False
